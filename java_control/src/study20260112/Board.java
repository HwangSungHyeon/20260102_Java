...
public class Board {
	...
	public void hitUp( String user ) {
		// writer 는 해당글의 작성자, user 는 글을 열람한 사람
...
3. 반환(return)만 있는 메서드
   public String 메서드이름( ) { return String 변수; }
...
특정 기능을 실행 되게 해주는 것.
( 기능이라하면 출력, 계산, 데이터 필터, 데이터 찾기,
  데이터 옮기기, 데이터 삭제 등)
  
  int[] bingo = new int[25];
  
  코드가 연결된 상태에서 잘못되면 다 갈아엎어야 하는 경우가 많지만
  메서드를 사용하면 오류가 난 부분만 고칠 수 있어 유용하다.
  intData(); // 무작위 숫자넣기
  whil(true){
      drawBingo(); 
      numberInput();
...
// 여기서 return 에 있는 tot의 타입이 int 이므로 무조건 int와 동일해야한다.
public int total() {
   int 55, mat =60, eng= 70;
